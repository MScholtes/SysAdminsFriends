...
.Example
Set-InFile -Pattern "Mister" -Replacement "Lady" -Path Test.txt -Quiet > result.txt

Replaces Mister with Lady in the file "Test.txt" and writes the result to result.txt
.Example
dir | Set-InFile -Pattern "12" -Replacement "34" -Overwrite

Replaces 12 with 34 in all files of the current directory
.Example
gci | Set-InFile -Pattern "late" -Replacement "later" -CaseSensitive -Recurse -OEM
...
.Example
Get-ChildItem "*.txt" | Set-InFile -Pattern "t$" -Replacement "T" -Encoding UNICODE -Overwrite
...
.Example
Get-ChildItem "*.txt" | Set-InFile -Pattern "t\r\n" -Replacement "T\r\n" -Overwrite

Replaces t at the end of the line with T in all txt files of the current directory.
.Example
"*.txt" | Set-InFile -Pattern "<NL>" -Replacement "`n" -enc "Ascii" -VERBOSE
...
.Example
Set-InFile -Pattern "away" -Path "*.txt" -Overwrite -PreserveDate -WhatIf
...
.Example
Set-InFile "*" -patt "Search" -repl "Replace" -rec -enc UTF8 -u -over
...
function Set-InFile
{
	...
	PROCESS
	{
		...
		{ # if it is a String, determine complete path name and object
			...
			{ # call Set-InFile for every file
			  foreach ($FileName in $FileObject) { Set-InFile -Pattern "$Pattern" -Replacement "$Replacement" -Path "$FileName" -CaseSensitive:$CaseSensitive -Unix:$Unix -Overwrite:$Overwrite -Force:$Force -PreserveDate:$PreserveDate -OEM:$OEM -Encoding $Encoding -Recurse:$Recurse -Quiet:$Quiet }
			  # and end function
			  return
		 	}
		}
	    ...
	    { # recursion?
	    	# yes -> then call Set-InFile with child objects
	    	# no -> do nothing
	      if ($Recurse)
	     	{ Get-ChildItem $Name | Set-InFile -Pattern "$Pattern" -Replacement "$Replacement" -CaseSensitive:$CaseSensitive -Unix:$Unix -Overwrite:$Overwrite -Force:$Force -PreserveDate:$PreserveDate -OEM:$OEM -Encoding $Encoding -Recurse -Quiet:$Quiet }
	    }
			else
